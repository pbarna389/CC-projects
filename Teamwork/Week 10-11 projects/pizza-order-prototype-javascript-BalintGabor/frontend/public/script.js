...
const pizzaCardComponent = (id, name, listItems, sizes, price, category) => {
    return `<div class="container menu-items filterDiv show ${category}">
                <div class="pizza" data-id=${id}>
                <img src="./public/img/pizza${id}.png" class="pizza-image" alt="" data-pic=${id}>
...
const pizzaComponent = (data, section) => {
    data.forEach(element => {
        ...
        section.insertAdjacentHTML('beforeend', pizzaCardComponent(`${element.id}`, `${element.pizza_name}`, ingredientsComponent(`${element.ingredients.join(", ")}`, `${element.id}`), sizes, amountAndPriceComponent(element.price, element.id), element.category));
